...
class c_products extends SWeb{
    ...
    public function index(){
    		$this->assign('css', $this->langs['productcss']) ; //首页标题
    		$this->assign('z_active3','z-active');
    ...
    private function show_product(){
		...
		if(!$product OR !array_key_exists($product['cat_id'], $this->pcats_ok)){
			...
			if($gimages){
				...
				if(substr($gimages, -5) != '</ul>') $gimages .= '</tr></table></ul>'; //如果没有</ul>封闭加上
			}
    ...
    private function show_category(){
		...
		//$this->assign('products', $products,'productList.tpl');



		//$this->assign('productList', $products); //分配分类产品

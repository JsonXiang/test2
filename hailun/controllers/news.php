...
class c_news extends SWeb{
    ...
    public function index(){
		@require(ROOT.'config/config.php');
    		$this->assign('z_active4','z-active');
    		$this->assign('css', $this->langs['newscss']) ; 
    		$this->assign('css2', $this->langs['aboutUScss']) ; 
    ...
    private function show_list(){
		...
		if(IS_CHINESE){
			...
			//$sql = "select a_id, title, lclicks, created from hailun_article WHERE is_show = 1 ORDER BY sort DESC limit 0,10";
		}else{
			
			//$sql = "select  a_id, title_en AS title, clicks, createdfrom hailun_article WHERE is_show = 1 ORDER BY sort DESC limit 0,10";

...
class c_articles extends SAdmin{
	...
	private function DeleteProductById($pro_id) {
		$product = $this->db->getOne("SELECT pro_id, cat_id, path, filename FROM " . TABLE_PREFIX . "article WHERE pro_id = '$pro_id'");

		if(empty($product)) return;

		$this->db->exe("DELETE FROM " . TABLE_PREFIX . "article WHERE pro_id = '$pro_id'");
		$this->db->exe("UPDATE " . TABLE_PREFIX . "acat SET counts = (counts-1) WHERE cat_id = '$product[cat_id]'");
	...
	public function save(){
		...
		if($pro_id){//编辑产品
			...
			Success('articles/edit?pro_id=' . $pro_id);
		}else{//添加产品
			$filesize = $imagefile['size'];
			if($filesize == 0)	{
			...
			if($this->UploadImage($imagefile, $file_path, $imagename)){
				...
				$this->db->exe("INSERT INTO " . TABLE_PREFIX . "article (cat_id, is_show, is_best, userid, username, path, filename, price, price_en, title, title_en, content, content_en, keywords, keywords_en, clicks, created) VALUES ('$cat_id', '$is_show', '$is_best', '$userid', '$username', '$file_path', '$imagename', '$price', '$price_en', '$title', '$title_en', '$content', '$content_en', '$keywords', '$keywords_en', '0', '$time') ");

				$lastid = $this->db->insert_id;
				$this->db->exe("UPDATE " . TABLE_PREFIX . "article SET sort = '$lastid' WHERE pro_id = '$lastid'");
				$this->db->exe("UPDATE " . TABLE_PREFIX . "acat SET counts = (counts+1) WHERE cat_id = '$cat_id'");
			...
			Success('articles/edit?pro_id=' . $lastid);
		}
	}
	...
	public function index(){
		...
		if($search OR $time OR $cat_id){
			SubMenu('产品列表', array(array('添加产品', 'articles/add'), array('全部产品', 'articles')));
		}else{
			SubMenu('产品列表', array(array('添加产品', 'articles/add')));
		}
		...
		$getproducts = $this->db->query("SELECT pro_id, sort, cat_id, is_show, is_best, userid, username, path, filename, title, title_en, clicks, created FROM " . TABLE_PREFIX . "article " . $Where . " ORDER BY is_show ASC, sort DESC LIMIT $start, $NumPerPage");
		$maxrows = $this->db->getOne("SELECT COUNT(pro_id) AS value FROM " . TABLE_PREFIX . "article " . $Where);

		echo '<form method="post" action="'.BURL('articles/updateproducts').'" name="productsform">';
		...
		if($maxrows['value'] < 1){
			...
			while($product = $this->db->fetch($getproducts)){
				...
				TableRow(array('<input type="hidden" name="pro_ids[]" value="'.$product['pro_id'].'" /><input type="text" name="sorts[]" value="' . $product['sort'] . '" size="4" />',
					...
					'<a href="'.BURL('articles/edit?pro_id='.$product['pro_id']).'">

					
				

					<img src="'. GetImageURL($product['path'], $product['filename']).'" width="40" onMouseMove="ShowBigImage();"></a>',




					'<a href="'.BURL('articles/edit?pro_id='.$product['pro_id']).'" title="英文: '.$product['title_en'].'">' . $title . '</a>',
					...
					Iif($product['is_show'], '<a href="'.URL('articles?id=' . $product['pro_id']).'" target="_blank"><img src="' . SYSDIR . 'public/admin/images/view.gif"></a>', '<img src="' . SYSDIR . 'public/admin/images/disview.gif">'),
	...
	public function add(){
		...
		if($pro_id){  //编辑时的
			SubMenu('产品管理', array(array('添加新闻', 'articles/add'),array('编辑新闻', 'articles/edit?pro_id='.$pro_id, 1),array('新闻列表', 'articles')));
			$product = $this->db->getOne("SELECT * FROM " . TABLE_PREFIX . "article WHERE pro_id = '$pro_id'");
			...
			SubMenu('产品管理', array(array('添加新闻', 'articles/add', 1),array('新闻列表', 'articles')));
		...
		<form id="editorform88" name="editorform88" method="post" enctype="multipart/form-data" action="'.BURL('articles/save').'">
			...
			<script type="text/javascript">
				jQuery(document).ready(function() {
					$("#main a.open").click(function(e){
						...
						$.dialog({title: "查看及确认删除", lock:true, content:"<img width=\'400\' height=\'400\' src=\''. SYSDIR .'newsPic/" + filename + "\'>",okValue:"  删除  ", ok:function(){
								ajax("' . BURL('articles/ajax?action=deleteone') . '", {file: filename}, function(data){
									_me.parent().hide();
								});
					...
					$("#main a.ajax").click(function(e){
						...
						ok:function(){
							ajax("' . BURL('articles/ajax?action=deletelast') . '", {}, function(data){
		...
		PrintSubmit(Iif($pro_id, '保存更新', '添加新闻'));
	}

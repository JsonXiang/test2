...
protected function map_cats($pcat_ids, $pid = 0,$level=0){
		//$sReturn = '';
		static $sReturn = array();
		foreach($pcat_ids as $cat_id => $p_id){
			if($pid == $p_id){
				...
				$sReturn[$cat_id]['url']=URL('products?cat=' . $cat_id);
				$sReturn[$cat_id]['name']=$this->pcategories[$cat_id]['name'];
				$sReturn[$cat_id]['level']=$level;
				$sReturn[$cat_id] ['cat_id']= $cat_id;
				$sReturn[$cat_id]['p_id']= $p_id;
				//$sReturn[$cat_id]['chilld'] = 
				/*foreach($sReturn as $k=>$v)
				{
					if($v['cat_id'] == $)
				}*/
				...
				{
				    // $sReturn[$cat_id]['chilld']  =  $this->map_cats($pcat_ids, $cat_id,$level+1);

					$sReturn = $this->map_cats($pcat_ids, $cat_id,$level+1);
				}
